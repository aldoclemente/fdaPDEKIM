                         ...
                         add = FALSE, main = NULL, col = "red", ...)
{
   ...
   if (is.null(time_locations) && is.null(locations)) {
      ...
      solution <- .Call("eval_FEM_time_nodes",N,x$mesh_time,time_locations,x$coeff[,lambdaS,lambdaT],x$FLAG_PARABOLIC, PACKAGE = "fdaPDEKIM")
...
plot.mesh.3D<-function(x,...){
 ...
 image.FEM.time = function(x,t,lambdaS=1,lambdaT=1,num_refinements=NULL,...)
 {
   ...
   solution <- .Call("eval_FEM_time_nodes",N,x$mesh_time,t,x$coeff[,lambdaS,lambdaT],x$FLAG_PARABOLIC, PACKAGE = "fdaPDEKIM")
   for(i in 1:length(t))
   {
     plot = FEM(solution[(1+(i-1)*N):(N+(i-1)*N)],x$FEMbasis)
     image.FEM(plot,num_refinements,...)
